...
public class CreativeEnergySourceRenderer implements BlockEntityRenderer<CreativeEnergySourceEntity> {

    public CreativeEnergySourceRenderer(BlockEntityRendererFactory.Context context) {
    }
    ...
    public void render(CreativeEnergySourceEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        MinecraftClient.getInstance().getItemRenderer().renderItem(CreativeEnergySourceEntity.REDSPEEN, ModelTransformation.Mode.NONE, LookingGlassClient.LIGHTMAP_MAX_LUMINANCE, overlay, matrices, vertexConsumers, 0);
        matrices.pop();
    }
}
